...
def solve_instance(filepath, log_dir, solver_class, settings, mode):
    ...
    if mode == "train":
        ...
        solver.set_search_PoolSolutions(max_solution=settings['max_solutions'])
    else:
        solver.set_aggressive()
    
    # Configure log path
    log_path = os.path.join(log_dir, f'{os.path.basename(filepath)}.log')
    solver.solve(log_file=log_path, time_limit=settings['max_time'], threads=settings['threads'])
...
def process_files(queue, input_dir, output_dirs, solver_class, settings, mode):
...
def get_parser():
    ...
    # Task parameters
    parser.add_argument('--task', choices=TASKS, default='CA',
    ...
    parser.add_argument('--mode', type=str, default='test',
                      help='Processing mode: train or test (default: %(default)s)')
    parser.add_argument('--difficulty', type=str, default='hard',
                        help='Difficulty level of the dataset (e.g., easy, medium, hard) (default: %(default)s)')
...
def main():
    ...
    solve_time = f'{args.max_time}s'
    input_dir = os.path.join(args.data_dir, mode, task, difficulty)
    output_dir = os.path.join(args.output_dir, mode, task, difficulty, solve_time)
    ...
    for _ in range(args.workers):
        p = Process(
            ...
            args=(file_queue, input_dir, output_dirs, 
                 SOLVER_CLASSES[args.solver], solver_settings, mode)
        )
        p.start()
        processes.append(p)
